...
}elseif($_GET['SVN_UPDATE']){

	$update_num = svn_update(UPDATES_PATH);

	$_SESSION['UPDATE_NUM'] = $update_num;
	redir('panel','svn_display'); die();


// Checkout the Working directory if not exists
}elseif($_GET['SVN_CHECHOUT']){

	svn_checkout(SVN_URL,UPDATES_PATH);
	redir('panel','svn_display'); die();

}elseif(isset($_GET['SVN_DISPLAY'])){

	$diff_files = diff_updated_files();
