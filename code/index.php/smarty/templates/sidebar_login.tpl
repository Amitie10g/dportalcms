...
{{if $PHPBB_URL_PATH != null}}<p style="text-align:center;margin:0"><a href="{{$PHPBB_URL_PATH}}ucp.php?mode=register">{{$LANG.register|ucfirst}}</a></p>{{else}}<input type="hidden" name="redir" value="{{$smarty.server.REQUEST_URI}}" />{{/if}}
</form>
