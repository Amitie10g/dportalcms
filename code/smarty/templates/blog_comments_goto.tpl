{{if $PAGE > 1}}
		...
		{{if $PREV == 1}}<a href="{{LINK script="blog_entry" section=$NAME}}">{{else}}<a href="{{LINK script="blog_comments_goto" section=$NAME  page=$PREV}}">{{/if}}≪&nbsp;Previous&nbsp;</a>&nbsp;{{/if}}
		...
		{{/if}}
		{{/section}}
		{{if $PAGE < $smarty.section.pages.total}}<a href="{{LINK script="blog_comments_goto" section=$NAME page=$NEXT}}">&nbsp;Next&nbsp;≫</a>
		</div>
{{/if}}
