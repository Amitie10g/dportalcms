...
function redir($script,$section,$http_status = null,$argument = null,$marker = null){
	...
	if($use_rewrite){
		...
		switch($script){
			...
			case "video"		: header("location: http://".$_SERVER['SERVER_NAME'].DPORTAL_PATH ."/tv/$section/" . $argument . ".html$marker"); break;
...
function link_url($params,&$smarty){
	...
	if($use_rewrite){
		...
		switch($script){
			...
			case 'player'			: $uri = 'http://'.$_SERVER['SERVER_NAME'].DPORTAL_PATH."/tv/".$page.'/'. $section.'.html'.$marker; break;
			case 'player_hq'		: $uri = 'http://'.$_SERVER['SERVER_NAME'].DPORTAL_PATH."/tv/".$page.'/'. $section.'_hq.html'.$marker; break;
			...
			case 'video_download'		: $uri = 'http://'.$_SERVER['SERVER_NAME'].DPORTAL_PATH."/tv/".$page.'/download:'. $section.'.flv'; break;
