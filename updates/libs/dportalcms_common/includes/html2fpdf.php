...
//////////////////////////////////////////////////////////////////////////////
//////////////////Added by Davod for DPortal CMS (see above)//////////////////
//////////////////////////////////////////////////////////////////////////////
//                                                                          //
// Important: I made few changes and additions to this file specifically to //
// this project. Please don't use this version of html2fpdf outside of      //
// DPortal CMS in Production, UNLESS you know doing, because changes may be //
// incompatible with other applications that use FPDF anf html2fpdf.        //
//                                                                          //
// Really REALLY important: FPDF_PATH and FPDF_FONTPATH constants are       //
// defined in '/config/config.php' in dportal CMS!!!                        //
//                                                                          //
// FPDF libraries directory is placed in 'libs' or the shared               //
// PHP directory in Debian version. Please see /config/constants.php to     //
// view the actual ubication of these files.                                //
//                                                                          //
// html2fpdf.php, gif.php and htmltoolkit.php are placed in '/includes'.    //
// license.txt and credits.txt are placed in '/readmes/' as html2fpdf_*     //
//                                                                          //
// Copyright remains on the original author! (see above)                    //
//                                                                          //
//////////////////////////////////////////////////////////////////////////////
...
function HTML2FPDF($orientation='P',$unit='mm',$format='letter')
{
//! @desc Constructor
...
// Added by Davod
function SetHeaderContent($content,$use_line = false){

	$this->header_content = $content;
	$this->use_header_line = $use_line;

}

// Added by Davod
// string from, string notice, bool use_line = false, string URL
function SetFooterContent($content,$use_line = false){

	$this->footer_content = strip_tags($content);
	$this->use_footer_line = $use_line;

}

function SetURLReferer($url){
...
// Added by Davod
function SetLicense($text,$url){

	$this->license_text = strip_tags($text);
	$this->license_url = $url;
...
function Header()
{
	...
	//Framed title
	$this->Cell(0,5,$this->header_content,0,0,'C');
	$this->Cell(0,5,$this->author,0,0,'R');
	if($this->use_header_line) $this->Line($this->lMargin,($this->tMargin + 5),($this->fw - $this->rMargin),($this->tMargin + 5));
	//Line break
	$this->Ln(7);
}
...
function Footer()
{
	...
	$this->SetY(-15);
	$this->SetTextColor(0);
	if($this->use_footer_line) $this->Line($this->lMargin,(($this->fh - $this->tMargin) - 5),($this->fw - $this->lMargin),(($this->fh - $this->tMargin) - 5));
	$this->SetFont('Arial',null,10);
	$this->Write(6,'From '.SITENAME. '. '.$this->license_text);
	$this->Cell(0,6,$this->PageNo().'/{nb}',0,0,'R');
