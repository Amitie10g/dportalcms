...
function edit_section($filename,$title,$content,$timestamp,$exclusive = null,$category = null){
	...
	$list = fopen(CONTENT_PATH . ".list",'wb') or die('Missing or inaccesible Filelist');
	flock($file,2); flock($list,2);

	// Ask if the operations are successful
	if(gzwrite($file,$output) && fwrite($list,$put_list)) $updated = true;
	
	// Closes the opened files
	gzclose($file); fclose($list);
...
function delete_section($filename){
	...
	$list = fopen(CONTENT_PATH . ".list",'wb');	flock($list,2);

	if(file_exists(CONTENT_PATH . $filename)) $unlinked = unlink(CONTENT_PATH . $filename);
