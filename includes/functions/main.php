...
function get_sections($category = null){
	...
	while (($data = fgetcsv($secs, 1000, ";")) !== FALSE){
		...
		if((($category == $category_file && $category != null) || ($category == null && strpos($name,"_") === false)) || $all) $sections[] = array('name'=>$name,'title'=>$title,'filename'=>CONTENT_PATH . $name);
	}
	fclose($secs);
