...
{{if $PHPBB_URL_PATH != null}}<a href="{{$PHPBB_URL_PATH}}ucp.php?mode=register">{{$LANG.register|ucfirst}}</a>{{else}}<input type="hidden" name="redir" value="{{$smarty.server.REQUEST_URI}}" />{{/if}}</p>
</form>
