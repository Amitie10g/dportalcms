...
function create_category($name,$title){
	...
	if($check_name>=1&&$check_title>=1){
		...
		$list = fopen(CONTENT_PATH.'.categories', "rb") or die("Missing or inaccesible List file! Please be sure that the file exist and is writable.");
...
function edit_section($filename,$title,$content,$timestamp,$exclusive = null,$category = null){
	...
	$output = wordwrap(preg_replace(array("/..\/editor\//","/target=\"[_\w]+\"/"),array($curr_dir.'editor/',"rel=\"external\""),stripslashes($content)));
...
function delete_section($filename){
	...
	if(!is_file(CONTENT_PATH . $filename) && !nofakedir($filename)) return false;
...
// bool delete_category(string category_name)
function delete_category($category_name){
	...
	while (($data = fgetcsv($list_file, 1000, ";")) !== FALSE) {
		if($data[0] == $category_name){
			...
			// Get the List of sections, with the given category
			$sections = get_sections($category_name);


			if($sections != null){
				// Loop through the Sections list, and Delete the given sections
				foreach($sections as $item){
					if(!is_file(CONTENT_PATH . $item['name'])) continue;
					if(!delete_section($item['name'])) return false;
				}
			}
