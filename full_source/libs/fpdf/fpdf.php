	...
	switch($dest)
	{
		case 'I':
			...
			if(isset($HTTP_SERVER_VARS['SERVER_NAME']))
			{
				...
				Header("Content-disposition: inline; filename='$name'"); // Don't forgot to add (escaped) quotes in Filename
			}
			echo $this->buffer;
			break;
		case 'D':
...
function _parsepng($file)
{
	...
	else $this->Error('Alpha channel not supported: '.$file);// Please supress this error!!!
...
{ 
	...
	require_once(INCLUDES_PATH.'gif.php'); //GIF class in pure PHP from Yamasoft (http://www.yamasoft.com/php-gif.zip)
